...
def equations(vars):
    ...
    f4 = (1.3 + 1.857e-3 * V5 ** 2 - 1.748e-3 * V5 * aunp.cos(theta5 - theta2) + 4.033 * V5 * aunp.sin(theta5 - theta2) -
          1.799e-4 * V5 * aunp.cos(theta5 - theta4) + 1.574 * V5 * aunp.sin(theta5 - theta4))
    ...
    f6 = (0.4 + 5.362 * V5 ** 2 - 1.748e-3 * V5 * aunp.sin(theta5 - theta2) -4.033 * V5 * aunp.cos(theta5 - theta2)
          - 1.799e-4 * V5 * aunp.sin(theta5 - theta4) - 1.574 * V5 * aunp.cos(theta5 - theta4))
...
def fast_decoupled_method(equations, initial_vars, max_iterations=100, tolerance=1e-6):
    vars_i = initial_vars.copy()
    ...
    for iteration in range(max_iterations):
        ...
        for i in range(6):
            for j in range(6):
                delta = 1e-8
                vars_plus = vars_i.copy()
                vars_minus = vars_i.copy()

                vars_plus[j] += delta
                vars_minus[j] -= delta

                f_plus = equations(vars_plus)
                f_minus = equations(vars_minus)

                J[i, j] = (f_plus[i] - f_minus[i]) / (2 * delta)

        # Calculate the update using the fast decoupled method
        update = np.linalg.solve(J, -equations(vars_i))
...
method_choice = "fast_decoupled"

if method_choice == "newton_raphson":
    result, iterate = multivariable_newton_raphson(initial_vars)
elif method_choice == "fast_decoupled":
    result, iterate = fast_decoupled_method(equations, initial_vars)
else:
